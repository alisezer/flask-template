...
This will install the required packages withing your virtualenv.
...
### Main Modules
Every flask application has a high level module for creating the app itself, in this case,this module is the `stories.py`. This contains the flask application, and is used by other services such as Gunicorn or Flask's CLI while serving the application.
...
`app/__init__.py` ties the necessary packages such as your SQLAlchemy or Migrations packges to your app, and provides a nice function for generating an application through with a prespecified config.
...
Usually for smaller projects, databases such as SQLite is preferred for the ease of use. However, in most of the production environments, these database are never used so learning how to set them up might because useless for larger commercial projects.

Keeping this in mind, even though the project is quiet small, I went the extra mile to setup a proper PostgreSQL database. The configratuions for this database is specified under the `.env` file, and it is set as the default database.
...
Make sure you save these informations, and add them to your `.env` file so your code can make changes to the database
...
You can also run your app using [Gunicorn](http://gunicorn.org/), which a seperate WSGI Server thats plays very well with Flask:
```
